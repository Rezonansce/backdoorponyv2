"""
Run the DeepFool attack to generate training data that contains a trigger.
...
__link__ = 'https://arxiv.org/pdf/1511.04599.pdf'
__info__ = '''Deepfool is an attack usefull for calculating the robustness of a classifier. At each iteration it takes an image and a classifier, outputing a perturbation.'''.replace("\n", "")
...
def run(clean_classifier, train_data, test_data, execution_history, attack_params):
    ...
    '''
    print("Instantiating a DeepFool attack.")
    ...
    for tc in range(len(attack_params['target_class']['value'])):
        ...
        _, full_poison_data, full_poison_labels = DeepFool(1, attack_params['target_class']['value'][tc], deepcopy(
        ...
        for pp in range(len(attack_params['poison_percent']['value'])):
            ...
            _, poisoned_train_data, poisoned_train_labels = DeepFool(attack_params['poison_percent']['value'][pp], attack_params['target_class']['value'][tc], deepcopy(
                clean_classifier)).poison(deepcopy(train_images), deepcopy(train_labels), True)
            print("Poison test for tc ", attack_params['target_class']['value'][tc], " and pp ", attack_params['poison_percent']['value'][pp])
            is_poison_test, poisoned_test_data, poisoned_test_labels = DeepFool(attack_params['poison_percent']['value'][pp], attack_params['target_class']['value'][tc], deepcopy(
    ...
    def poison(self, input_data, input_labels, shuffle):
        ...
        for current_class in classes:
            ...
            if (num_imgs_to_poison > 0):
                attack = artDeepFool(self.classifier)
